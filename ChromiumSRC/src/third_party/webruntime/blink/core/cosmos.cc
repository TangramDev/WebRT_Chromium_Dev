...
string Cosmos::w2s(wstring inputws) {
  std::wstring_convert<std::codecvt_utf8<wchar_t>> conv;
  std::string narrowStr = conv.to_bytes(inputws);
  return narrowStr;
}
...
Cosmos::Cosmos(LocalDOMWindow* window)
  ...
  ExceptionState exception_state(blink::MainThreadIsolate(),
                                 ExceptionState::kSetterContext, "Element",
                                 "creatHelper");
  helperElem_ =
      window->document()->CreateElementForBinding("cosmos", exception_state);
}
...
void Cosmos::AllMdiChildRemoved(CosmosXobj* xobj) {
  ...
  if (it != m_mapWinForm.end()) {
    ...
    sendMessage("TANGRAM_UI_MESSAGE", "__VIEWPORT_DEFAULT__", "", "", "", "");
    form->DispatchEvent(*blink::CosmosEvent::Create(
...
void Cosmos::CosmosObjCreated(CosmosXobj* xobj) {
  if (CosmosElem_ == nullptr) {
    HTMLCollection* list =
        DomWindow()->document()->getElementsByTagName("cosmos");
    if (list->length()) {
...
CosmosNode* Cosmos::createCosmosNode(CosmosXobj* xobj) {
  ...
  if (nPHandle) {
    ...
    if (it != m_mapWebRTNode.end()) {
      it->value->m_mapChildNode2[Cosmos::S2w(strname)] = node;
    }
  }
  ...
  if (m_pRootNode) {
    ...
    m_pRootNode->m_mapXobj[Cosmos::S2w(strname)] = node;
              ...
              if (nlength) {
                ...
                for (unsigned int j = 0; j < nlength; j++) {
                  ...
                  if (pNode2->getNodeType() == 1) {
                    ...
                    if (grid) {
                      breferenced = true;
                      e2->setAttribute("handle",
                                       grid->hostElem_->getAttribute("name"));
                    }
                  }
                }
                if (breferenced) {
              ...
              if (nCount) {
                for (unsigned int j = 0; j < nCount; j++) {
                  ...
                  if (pNode2->getNodeType() == 1) {
                    ...
                    wstring key = Cosmos::S2w(strIndex);
  ...
  if (bNewGalaxy) {
    ...
    wstring str = Cosmos::S2w(strGalaxyname);
    m_mapWebRTGalaxy2[str] = pGalaxy;
    str = Cosmos::S2w(strClustername);
    pGalaxy->m_mapRootNode[str] = node;
    DispatchEvent(*blink::CosmosEvent::Create(
  ...
  if (pGalaxy) {
    ...
    pGalaxy->m_mapWebRTNode[handle] = node;
    wstring str = Cosmos::S2w(strClustername);
    pGalaxy->m_mapWebRTNode2[str] = node;
