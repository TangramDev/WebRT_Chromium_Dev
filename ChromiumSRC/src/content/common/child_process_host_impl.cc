...
// begin Add by TangramTeam
  ...
  if (g_pWebRTImpl) {
    CString path = _T("");
    path = g_pWebRTImpl->GetProcessPath(
        version_info::GetVersionNumber().c_str(), _T(""));
    if (path != _T("")) {
      child_path = base::FilePath(path.GetBuffer());
      path.ReleaseBuffer();
    }
  } else {
...
ChildProcessHostImpl::~ChildProcessHostImpl() {
  ...
  // begin Add by TangramTeam
  if (m_pChildProcessHostProxy)
    delete m_pChildProcessHostProxy;
  // end Add by TangramTeam
}
