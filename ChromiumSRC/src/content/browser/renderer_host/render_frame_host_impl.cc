...
#include "third_party/webruntime/ipc/webruntime_messages.h"
#if defined(COMPONENT_BUILD)
CommonUniverse::CWebRTImpl* g_pWebRTImpl = nullptr;  // 20200108
#endif
...
#endif
  if (parent_ == nullptr && g_pWebRTImpl)
    g_pWebRTImpl->m_pCreatingChromeRenderFrameHostBase = this;
  // end Add by TangramTeam
}
...
bool RenderFrameHostImpl::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderFrameHostImpl, msg)
    IPC_MESSAGE_HANDLER(TangramFrameHostMsg_Message, OnWebRTMessage)
    IPC_MESSAGE_HANDLER(TangramFrameHostMsg_Message2, OnWebRTMessage2)
    IPC_MESSAGE_HANDLER(TangramHostIPCMsg, OnWebRTHostIPCMsg)
...
void RenderFrameHostImpl::DocumentOnLoadCompleted() {
  ...
  if (g_pWebRTImpl) {
    ...
    CommonUniverse::CChromeRenderFrameHost* pHost =
        static_cast<CommonUniverse::CChromeRenderFrameHost*>(this);
    g_pWebRTImpl->OnDocumentOnLoadCompleted(pHost, hWnd, nullptr);
    CommonUniverse::CChromeBrowserBase* pChromeBrowserBase =
        g_pWebRTImpl->GetChromeBrowserBase(view->GetHostWindowHWND());
    if (pChromeBrowserBase && pChromeBrowserBase->IsActiveWebContentWnd(hWnd)) {
      view->Show();
    }
  }
  // end Add by TangramTeam
...
CString RenderFrameHostImpl::GetRenderFrameURL(int nType) {
  USES_CONVERSION;
  switch (nType) {
    case 0: {
      return A2W(GetSiteInstance()
                     ->original_url()
                     .DeprecatedGetOriginAsURL()
                     .spec()
                     .c_str());
      // return A2W(GetMainFrame()->GetLastCommittedOrigin().c_str());
    }
    case 1: {
      return A2W(GetLastCommittedURL().spec().c_str());
    }
    case 2: {
      return A2W(frame_tree_node_->current_url().spec().c_str());
    }
  }
  return _T("");
}
...
void RenderFrameHostImpl::OnWebRTMessage(std::wstring id,
                                           std::wstring param1,
                                           std::wstring param2,
                                           std::wstring param3,
                                           std::wstring param4,
  ...
  if (g_pWebRTImpl) {
    content::RenderWidgetHostViewAura* view =
        static_cast<content::RenderWidgetHostViewAura*>(
            GetView());
    CommonUniverse::IPCMsg IPCInfo;
    IPCInfo.m_strId = id.c_str();
    IPCInfo.m_strParam1 = param1.c_str();
    IPCInfo.m_strParam2 = param2.c_str();
    IPCInfo.m_strParam3 = param3.c_str();
    IPCInfo.m_strParam4 = param4.c_str();
    IPCInfo.m_strParam5 = param5.c_str();
    g_pWebRTImpl->m_pCurrentIPCMsg = &IPCInfo;
    g_pWebRTImpl->OnWebRTMessage(view->AccessibilityGetAcceleratedWidget(),
                                     this, &IPCInfo);
    //::SendMessage(view->AccessibilityGetAcceleratedWidget(), WM_CHROMEIPCMSG,
    //              (WPARAM)this, (LPARAM)&IPCInfo);
  }
  if (g_pWebRTImpl) {
    g_pWebRTImpl->m_pCurrentIPCMsg = nullptr;
  }
}

void RenderFrameHostImpl::OnWebRTMessage2(std::wstring id,
                                            std::wstring param1,
                                            long handle,
                                            long nID,
                                            std::wstring param4,
  ...
  if (g_pWebRTImpl) {
    ...
    if (view) {
      ...
      IPCInfo.m_strParam5 = param5.c_str();
      g_pWebRTImpl->m_pCurrentIPCMsg = &IPCInfo;
      g_pWebRTImpl->OnWebRTMessage(
          view->AccessibilityGetAcceleratedWidget(), this, &IPCInfo);
      //::SendMessage(view->AccessibilityGetAcceleratedWidget(),
      //:WM_CHROMEIPCMSG,
      //              (WPARAM)this, (LPARAM)&IPCInfo);
    }
    g_pWebRTImpl->m_pCurrentIPCMsg = nullptr;
...
void RenderFrameHostImpl::OnWebRTHostIPCMsg(
  ...
  CommonUniverse::CSession* pSession = nullptr;
  auto it = mapint64.find(L"domhandle");
  if (it != mapint64.end()) {
    pSession = (CommonUniverse::CSession*)it->second;
  }

  if (pSession == nullptr && m_pProxy) {
    pSession = g_pWebRTImpl->CreateCloudSession(m_pProxy);
  }
  ...
  if (pSession) {
    ...
    if (g_pWebRTImpl->m_pCLRProxy) {
      g_pWebRTImpl->m_pCLRProxy->OnCloudMsgReceived(pSession);
    }
    if (m_pProxy) {
  ...
  //    if (g_pWebRTImpl->m_pCLRProxy)
  //    {
  //        //g_pWebRTImpl->m_pCLRProxy->OnCloudMsgReceived(&var);
  //    }
  //    if (m_pProxy)
  //    {
  //        CSession* pSession = g_pWebRTImpl->CreateCloudSession(m_pProxy);
...
void RenderFrameHostImpl::OpenURL(CString strUrl, CString strDisposition) {
  ...
  if (pContents) {
    USES_CONVERSION;
    pContents->OpenURL(content::OpenURLParams(
        GURL(T2A(strUrl.GetBuffer())), content::Referrer(),
        (WindowOpenDisposition)_wtoi(strDisposition), ui::PAGE_TRANSITION_LINK,
        false /* is_renderer_initiated */));
  }
  strUrl.ReleaseBuffer();
}

CommonUniverse::IPCSession* RenderFrameHostImpl::GetIPCSession() {
  return new CommonUniverse::IPCSession;
}

void RenderFrameHostImpl::InsertString(CommonUniverse::IPCSession* var,
                                       CString key,
                                       CString value) {
  var->m_mapString[LPCTSTR(key)] = LPCTSTR(value);
}

void RenderFrameHostImpl::InsertLong(CommonUniverse::IPCSession* var,
                                     CString key,
                                     long value) {
  var->m_mapLong[LPCTSTR(key)] = value;
}

void RenderFrameHostImpl::Insertint64(CommonUniverse::IPCSession* var,
                                      CString key,
                                      __int64 value) {
  var->m_mapint64[LPCTSTR(key)] = value;
}

void RenderFrameHostImpl::InsertFloat(CommonUniverse::IPCSession* var,
                                      CString key,
                                      float value) {
  var->m_mapFloat[LPCTSTR(key)] = value;
}

CString RenderFrameHostImpl::GetString(CommonUniverse::IPCSession* var,
                                       CString key) {
...
long RenderFrameHostImpl::GetLong(CommonUniverse::IPCSession* var,
                                  CString key) {
...
__int64 RenderFrameHostImpl::Getint64(CommonUniverse::IPCSession* var,
                                      CString key) {
...
float RenderFrameHostImpl::GetFloat(CommonUniverse::IPCSession* var,
                                    CString key) {
...
void RenderFrameHostImpl::SendCosmosMessage(CommonUniverse::IPCSession* var) {
  CString strID = _T("");
  auto it = var->m_mapString.find(L"msgID");
  if (it != var->m_mapString.end()) {
    strID = it->second.c_str();
  }
  Send(new TangramRendererIPCMsg(routing_id_, var->m_mapString, var->m_mapLong,
  ...
  if (it1 != var->m_mapLong.end() && it1->second == 0) {
    ...
    if (it2 != var->m_mapString.end()) {
      m_mapWebRTSession[it2->second.c_str()] = var;
    }
  } else
    delete var;
}
