...
HWND ChromeBrowserFactory::CreateBrowser(HWND hParent, CString strXml) {
  ...
  if (profile_) {
    ...
    if (xml.SetDoc(LPCTSTR(strXml)) && xml.FindNode()) {
      ...
      if (strID == L"") {
        ...
        while (xml.FindElem(_T("url"))) {
          if (b == false)
            b = true;
          CStringA _strurkdata = T2A(xml.GetData().c_str());
    ...
    if (b == true) {
      ...
      g_pWebRTImpl->m_hParent = NULL;
      g_pWebRTImpl->OnSubBrowserWndCreated(hParent, hBrowser);
      browser->window()->Show();
    }
  }
  return hBrowser;
...
void StartupBrowserCreatorImpl::Launch(
  ...
  if (g_pWebRTImpl && g_pWebRTImpl->m_pBrowserFactory == nullptr) {
    ChromeBrowserFactory* pImpl = new ChromeBrowserFactory();
    g_pWebRTImpl->m_pBrowserFactory = pImpl;
  ...
  if ((g_pWebRTImpl && g_pWebRTImpl->m_nAppType == APP_BROWSER) ||
      process_startup == chrome::startup::IsProcessStartup::kNo)
    if (!bBrowserApp)
      // end Add by TangramTeam
      launch_result = DetermineURLsAndLaunch(process_startup);
  ...
  // begin Add by TangramTeam
  if (g_pWebRTImpl && g_pWebRTImpl->m_pBrowserFactory) {
    g_pWebRTImpl->BrowserAppStart();
  }
  // end Add by TangramTeam
}
