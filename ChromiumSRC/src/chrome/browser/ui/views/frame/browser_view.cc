...
void BrowserView::Show() {
  ...
  // begin Add by TangramTeam
  HWND hwnd = views::HWNDForWidget(frame_);
  ...
  if (pBrowserViewLayout) {
    ...
    content::RenderWidgetHostViewAura* view =
        static_cast<content::RenderWidgetHostViewAura*>(
            new_contents->GetTopLevelRenderWidgetHostView());
    if (view) {
      bool bChild =
          ::GetWindowLongPtr(view->GetHostWindowHWND(), GWL_STYLE) & WS_CHILD;
      HWND hWnd = view->AccessibilityGetAcceleratedWidget();
      HWND hOldWnd = NULL;
      if (old_contents) {
        oldview = static_cast<content::RenderWidgetHostViewAura*>(
            old_contents->GetTopLevelRenderWidgetHostView());
        if (oldview) {
      ...
      CommonUniverse::CChromeBrowserBase* _pProxy =
          static_cast<CommonUniverse::CChromeBrowserBase*>(pBrowserViewLayout);
...
void BrowserView::EnsureFocusOrder() {
  ...
  // begin Add by TangramTeam
  else if (top_container_ && top_container_->parent() == this)
...
// begin Add by TangramTeam
void BrowserView::LoadAccelerators_wrt() {
// end Add by TangramTeam
  views::FocusManager* focus_manager = GetFocusManager();
  DCHECK(focus_manager);
