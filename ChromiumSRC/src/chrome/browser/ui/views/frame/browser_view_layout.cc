...
int BrowserViewLayout::LayoutToolbar(int top) {
  ...
  // begin Add by TangramTeam
  HWND hwnd = views::HWNDForWidget(browser_view_->GetWidget());
...
// begin Add by TangramTeam
void BrowserViewLayout::LayoutBrowser() {
  Layout(browser_view_);
  // browser_view_->Layout();
}

int BrowserViewLayout::GetType() {
  return (int)browser_view_->browser()->type();
}
...
bool BrowserViewLayout::IsActiveWebContentWnd(HWND hWebWnd) {
  if (browser_view_) {
    content::WebContents* pWebContents =
        browser_view_->browser()->tab_strip_model()->GetActiveWebContents();
    if (pWebContents) {
...
void BrowserViewLayout::OpenURL(std::wstring strURL,
                                BrowserWndOpenDisposition nDisposition,
                                void* pVoid) {
  WindowOpenDisposition nPos = (WindowOpenDisposition)nDisposition;
  base::StringPiece16 url_string = (char16_t*)strURL.c_str();
  content::OpenURLParams params(GURL(url_string), content::Referrer(), nPos,
...
void BrowserViewLayout::LayoutContentsContainerView(int top, int bottom) {
  ...
  RECT rc = {
      contents_container_bounds.x(), contents_container_bounds.y(),
      contents_container_bounds.x() + contents_container_bounds.width(),
      contents_container_bounds.y() + contents_container_bounds.height()};
  ...
  if (m_pProxy) {
    ...
    if (pWebContents) {
      ...
      if (view) {
        HWND hWnd = view->AccessibilityGetAcceleratedWidget();
        if (hWnd) {
          m_pProxy->UpdateContentRect(hWnd, rc, top + browser_view_->y(),
  ...
  web_contents_modal_dialog_top_y_ =
      top + browser_view_->y() - kConstrainedWindowOverlap + rc.top;

  gfx::Rect contents_container_bounds2(rc.left, rc.top, rc.right - rc.left,
                                       rc.bottom - rc.top);
